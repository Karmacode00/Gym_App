...
public interface EvaluationDao {
    @Query("SELECT id, weight, date, imc, user_id FROM evaluations WHERE user_id = :userId")
    List<EvaluationEntity> findAll (long userId);

    @Query("SELECT id, weight, date, imc, user_id FROM evaluations WHERE user_id = :userId AND date BETWEEN :from AND :to")
